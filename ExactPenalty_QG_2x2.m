...
label_linear = zeros(length(y1),length(y2));
...
for i =1:length(y1)
    for j =1:length(y2)
        ...
        if lambda_max == 0
            ...
            obj(i,j)  = -b(1)*y1(i)-b(2)*y2(j)+rho*lambda_max + b.'*ystar;
            obj_linear(i,j) = nan;
        end
...
%label_linear = logical(label_linear);

f1 = figure;
firstax = axes (f1, 'FontSize', 16); 
h1 = surf(Y1,Y2,obj_linear,'EdgeColor', 'none','FaceColor','#EDB120','FaceAlpha',0.5);
%legend('$-b^{\top}y$','interpreter','latex','Location','none','Position',[0.05,0.87,0.37,0.1],'Box','off','FontSize', 11, 'NumColumns',2);

% 
hold on;
h2 = surf(Y1,Y2,obj,'EdgeColor', 'none','FaceColor','#4DBEEE','FaceAlpha',0.5);
...
%    'Location','none','Position',[0.2,0.87,0.37,0.1],'Box','off','FontSize', 11, 'NumColumns',3);
...
% h3 = surf(NaN(resolution),NaN(resolution),NaN(resolution),'EdgeColor', 'none','FaceColor','#4DBEEE','FaceAlpha',0.5,'Parent', secondax);
% %legend('$-b^{\top}y + \rho\max\{ \lambda_{\max}(\mathcal{A}^*y - C),0\}$','interpreter','latex','Location','none','Position',[0.5,0.87,0.37,0.1],'Box','off','FontSize', 11, 'NumColumns',2);
% hp =  legend(h3,'$-b^{\top}y + \rho\max\{ \lambda_{\max}(\mathcal{A}^*y - C),0\}$','interpreter','latex',...
%     'Location','none','Position',[0.5,0.87,0.37,0.1/2],'Box','off','FontSize', 11);
% % zl = zlim;
% % zlim([0,zl(2)]);
% xlabel('$y_1$','interpreter','latex');
% ylabel('$y_2$','interpreter','latex');
...
%legend('$-b^{\top}y$','$-b^{\top}y + \rho\max\{ \lambda_{\max}(\mathcal{A}^*y - C),0\}$','interpreter','latex','Location','none','Position',[0.05,0.87,0.37,0.1],'Box','off','FontSize', 11, 'NumColumns',2);
% legend('$-b^{\top}y + \rho\max\{ \lambda_{\max}(\mathcal{A}^*y - C),0\}$','$0.3 \cdot\mathrm{Dist}^2(y,S)$','interpreter','latex','Location','none','Position',[0.05,0.87,0.37,0.1],'Box','off','FontSize', 11);
